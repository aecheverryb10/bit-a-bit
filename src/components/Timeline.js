...
const Timeline = () => {
	...
	const moveTimeline = (section, index) => {
			...
			<nav className='flex gap-3'>
				{
					timeline_data.map((section, index_section) => {
						return (
							<button key={`button-${section.name}`} type='button' onClick={() => moveTimeline(section.name, index_section)}>
								{section.name}
							</button>
						);
					})
										...
										section.periods.map((period, index_period) => {
											...
											return (
												<div key={`${section}-${title}`} className='timeline-period'>
													<button className='relative' type='button' style={{height: 129, width: 129}}>
														<Image
