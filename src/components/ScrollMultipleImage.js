...
const ScrollMultipleImages = (props) => {
  ...
  useGSAP(
    () => {
      ...
      tl_images.scrollTrigger.refresh(true);
    },
    { scope: containerRef }
  );
    ...
    <div className='overflow-hidden' ref={containerRef}>
      <div className='flex relative'>
          ...
          <div className='relative' style={{ height: `${images.length + 1}00vh` }}>
            {images.map((item, index) => (
              <div
                className={`${
                  index === 0 ? 'opacity-100' : ''
                } absolute image--container-${index} h-screen w-full top-0`}
                key={`images-${item.key}-${index}`}
              >
                {/* eslint-disable-next-line @next/next/no-img-element */}
                <img className='image h-full w-full object-cover object-center ' alt={item.alt} src={item.img} />
              </div>
            ))}
        ...
        <div className='h-full absolute right-0'>
          {images.map((item, index) => (
            <div className='h-screen flex justify-end items-end pr-24' key={`caption-${item.key}-${index}`}>
              <div className='image-caption  bottom-0 text-white caption-element'>{item.caption}</div>
            </div>
          ))}
